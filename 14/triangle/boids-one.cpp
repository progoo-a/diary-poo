...
int main() {
    ...
    while (window.isOpen()) {
        ...
        boids[0].velocity += separation_factor * 0.1f;
        boids[0].velocity += (cohesion_factor - pos) * 0.01f;
        boids[0].velocity += alignment_factor * 0.1f;
        if (boids[0].velocity.x > 2) boids[0].velocity.x = 1;
